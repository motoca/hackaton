...
---                         | ---                           | ---
PyTorch                     | -                             | yolov5x.pt
TorchScript                 | `torchscript`                 | yolov5x.torchscript
ONNX                        | `onnx`                        | yolov5x.onnx
OpenVINO                    | `openvino`                    | yolov5x_openvino_model/
TensorRT                    | `engine`                      | yolov5x.engine
CoreML                      | `coreml`                      | yolov5x.mlmodel
TensorFlow SavedModel       | `saved_model`                 | yolov5x_saved_model/
TensorFlow GraphDef         | `pb`                          | yolov5x.pb
TensorFlow Lite             | `tflite`                      | yolov5x.tflite
TensorFlow Edge TPU         | `edgetpu`                     | yolov5x_edgetpu.tflite
TensorFlow.js               | `tfjs`                        | yolov5x_web_model/
PaddlePaddle                | `paddle`                      | yolov5x_paddle_model/
...
Usage:
    $ python export.py --weights yolov5x.pt --include torchscript onnx openvino engine coreml tflite ...
...
Inference:
    $ python detect.py --weights yolov5x.pt                 # PyTorch
                                 ...
                                 yolov5x.mlmodel            # CoreML (macOS-only)
...
TensorFlow.js:
    ...
    $ npm install
    $ ln -s ../../yolov5/yolov5x_web_model public/yolov5x_web_model
    $ npm start
"""
...
def export_openvino(file, metadata, half, int8, data, prefix=colorstr("OpenVINO:")):
    ...
    Examples:
        ...
        export_openvino(Path('yolov5x.onnx'), metadata={'names': model.names, 'stride': model.stride}, half=True,
...
def export_engine(
    ...
    Example:
        ...
        model = YOLOv5('yolov5x.pt')  # Load a pre-trained YOLOv5 model
        input_tensor = torch.randn(1, 3, 640, 640).cuda()  # example input tensor on GPU
        export_path = Path('yolov5x.engine')  # export destination
...
def pipeline_coreml(model, im, file, names, y, mlmodel, prefix=colorstr("CoreML Pipeline:")):
    ...
    Examples:
        ...
        model = torch.load('yolov5x.pt')  # Load YOLOv5 model
        ...
        output_file = Path('yolov5x.mlmodel')  # Convert to CoreML
...
def run(
    ...
    Args:
        ...
        weights (str | Path): Path to the pretrained model weights file. Default is 'yolov5x.pt'.
...
def parse_opt(known=False):
    ...
    parser.add_argument("--weights", nargs="+", type=str, default=ROOT / "yolov5x.pt", help="model.pt path(s)")
