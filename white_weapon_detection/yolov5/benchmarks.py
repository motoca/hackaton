...
CoreML                      | `coreml`                      | yolov5x.mlpackage
...
Usage:
    $ python benchmarks.py --weights yolov5x.pt --img 640
"""
...
def run(
    ...
    Args:
        weights (Path | str): Path to the model weights file (default: ROOT / "yolov5x.pt").
...
def test(
    ...
    Args:
        weights (Path | str): Path to the model weights file (.pt format). Default is 'ROOT / "yolov5x.pt"'.
...
def parse_opt():
    ...
    Args:
        weights (str): The path to the weights file. Defaults to 'ROOT / "yolov5x.pt"'.
