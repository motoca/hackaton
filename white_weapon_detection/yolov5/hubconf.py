...
Usage:
    import torch
    model = torch.hub.load('ultralytics/yolov5', 'yolov5x')  # official model
    model = torch.hub.load('ultralytics/yolov5:master', 'yolov5x')  # from branch
    model = torch.hub.load('ultralytics/yolov5', 'custom', 'yolov5x.pt')  # custom/local model
    model = torch.hub.load('.', 'custom', 'yolov5x.pt', source='local')  # local repo
"""
...
def _create(name, pretrained=True, channels=3, classes=80, autoshape=True, verbose=True, device=None):
    ...
    Args:
        name (str): Model name (e.g., 'yolov5x') or path to the model checkpoint (e.g., 'path/to/best.pt').
    ...
    Examples:
        ...
        # Load an official yolov5x model with pretrained weights
        model = _create('yolov5x')
        ...
        model = _create('yolov5x', channels=1, classes=10)
        ```
...
def custom(path="path/to/model.pt", autoshape=True, _verbose=True, device=None):
    ...
    Examples:
        ...
        model = torch.hub.load('ultralytics/yolov5', 'custom', 'yolov5x.pt')

        # Load model from a local path without autoshape on the CPU device
        model = torch.hub.load('.', 'custom', 'yolov5x.pt', source='local', autoshape=False, device='cpu')
        ```
    """
...
def yolov5x(pretrained=True, channels=3, classes=80, autoshape=True, _verbose=True, device=None):
    """
    Create a YOLOv5-small (yolov5x) model with options for pretraining, input channels, class count, autoshaping,
    ...
    Example:
        ...
        model = torch.hub.load('ultralytics/yolov5:master', 'yolov5x')
        ...
        model = torch.hub.load('.', 'custom', 'yolov5x.pt', source='local')
        ```
...
def yolov5x6(pretrained=True, channels=3, classes=80, autoshape=True, _verbose=True, device=None):
    ...
    Usage:
        ...
        model = torch.hub.load('ultralytics/yolov5:master', 'yolov5x6')  # load from a specific branch
        model = torch.hub.load('ultralytics/yolov5', 'custom', 'path/to/yolov5x6.pt')  # custom/local model
        model = torch.hub.load('.', 'custom', 'path/to/yolov5x6.pt', source='local')  # local repo model
        ```
