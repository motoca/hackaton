...
Usage - Single-GPU training:
    $ python train.py --data weapons.yaml --weights yolov5x.pt --img 640  # from pretrained (recommended)
    $ python train.py --data weapons.yaml --weights '' --cfg yolov5x.yaml --img 640  # from scratch

Usage - Multi-GPU DDP training:
    $ python -m torch.distributed.run --nproc_per_node 4 --master_port 1 train.py --data weapons.yaml --weights yolov5x.pt --img 640 --device 0,1,2,3
...
def train(hyp, opt, device, callbacks):
    ...
    Example:
        ...
        ```bash
        $ python -m torch.distributed.run --nproc_per_node 4 --master_port 1 train.py --data weapons.yaml --weights
        yolov5x.pt --img 640 --device 0,1,2,3
        ```
...
def parse_opt(known=False):
    ...
    parser.add_argument("--data", type=str, default=ROOT / "data/weapons.yaml", help="dataset.yaml path")
...
def run(**kwargs):
    ...
    Args:
        weights (str, optional): Path to initial weights. Defaults to ROOT / 'yolov5x.pt'.
        cfg (str, optional): Path to model YAML configuration. Defaults to an empty string.
        data (str, optional): Path to dataset YAML configuration. Defaults to ROOT / 'data/weapons.yaml'.
    ...
    Examples:
        ```python
        import train
        train.run(data='weapons.yaml', imgsz=320, weights='yolov5x.pt')
        ```
